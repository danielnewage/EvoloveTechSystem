...
const SalaryManageAuto = () => {
  ...
  // --- Modal and Salary Management ---
  // Check Firebase if salary for this month already exists and return the record (if any)
  ...
  // --- Automatic Salary Calculation based on Attendance Sheet ---
  const handleCalculateSalary = () => {
    if (!selectedEmployeeId) return;
    
    const records = getEmployeeRecordsForMonth(selectedEmployeeId);
  // Prevent salary calculation if any day has 'No Record'
  const hasNoRecord = records.some(r => r.status === 'No Record');
  if (hasNoRecord) {
    alert('Please complete attendance for all days before calculating salary.');
    return;
  }
    const fullLeaveDates = records
      .filter((record) => record.status === "Absent" || record.status === "No Record")
      .map((record) => normalizeDate(record.date))
      .sort();
    const fullLeave = calculateSandwichLeave(fullLeaveDates, publicHolidays);

    const lateArrivalCount = records.filter(
      (record) =>
        record.status === "Late Arrival" &&
        (!record.lateArrivalApproved || record.lateArrivalApproved.toLowerCase() !== "yes")
    ).length;
    const halfPresentCount = records.filter((record) => record.status === "Half Present").length;

    const extraLeave = computeExtraLeave(lateArrivalCount);

    const effective = fullLeave + (halfPresentCount * 0.5) + extraLeave;
  ...
  const exportReceiptPDF = () => {
    ...
    doc.text("Newage Dispatch", 45, 15);
    doc.setFontSize(14);
      ...
      <div className="overflow-x-auto shadow-lg rounded-lg border border-gray-200">
        <table className="min-w-full bg-white">
          ...
          <tbody className="divide-y divide-gray-200">
            {employees.map((emp) => {
              const empRecordsRaw = filteredAttendance.filter(
                (record) => record.employeeId === emp.id
              );
              const empRecords = empRecordsRaw.map((record) => {
                if (record.status === "Present" && record.timeIn !== "-") {
                  const minutes = timeToMinutes(record.timeIn);
                  if (minutes > 7 * 60 + 59) {
                    return { ...record, status: "Half Present" };
              ...
              const fullPresent = empRecords.filter((r) => r.status === "Present").length;
              const absent = empRecords.filter((r) => r.status === "Absent").length;
              const lateArrival = empRecords.filter((r) => r.status === "Late Arrival").length;
              const halfPresent = empRecords.filter((r) => r.status === "Half Present").length;
              ...
              return (
                <tr key={emp.id} className="hover:bg-gray-100 transition-colors">
                  ...
                  <td className="py-4 px-4 whitespace-nowrap text-center text-green-700 font-bold">{fullPresent}</td>
                  <td className="py-4 px-4 whitespace-nowrap text-center text-red-700 font-bold">{absent}</td>
                  <td className="py-4 px-4 whitespace-nowrap text-center text-purple-700 font-bold">{lateArrival}</td>
                  <td className="py-4 px-4 whitespace-nowrap text-center text-yellow-700 font-bold">{halfPresent}</td>
